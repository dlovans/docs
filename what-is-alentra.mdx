...
## 3. Workflows and sessions — define once, run safely

A **workflow** is the reusable, customer-ordered definition. A **session** is one
non-repeatable run of one published workflow revision. Build and publish the flow in
the dashboard, then launch it through a reusable link, as a one-time dashboard link,
or through the API with its `workflow_id`.

<Steps>
  <Step title="Create">
    Compose and order the primitives in the dashboard Workflows page, choose one launch
    mode, and publish. For API-only runs, call `POST /v1/sessions` with the published
    `workflow_id` and optional metadata. Alentra returns a session `id` and hosted `url`.
  </Step>
  <Step title="Hand off">
