...
A **session** bundles the steps you need into a single user experience. Create it through
the API or, without code, from the owner dashboard; then hand the user its **hosted URL**. On
...
<Steps>
  <Step title="Create">
    Use `POST /v1/sessions` with your `steps[]`, or compose the same supported steps on
    the dashboard Sessions page. Alentra returns a session `id` and hosted `url`.
  </Step>
  <Step title="Hand off">
