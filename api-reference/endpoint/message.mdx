...
<ParamField body="mode" type="string">
  Optional. How the audit record for this message is classified.

  **Valid values:** `plain`, `certified`

  **Default behavior** (when omitted):
  - If `signature_required: true` → defaults to `"certified"`
  - If `signature_required: false` → defaults to `"plain"`

  Most callers can omit this and let the default match the signature requirement. Override only when you need the audit's `mode` field to differ from the signature requirement (rare).
</ParamField>
