...
Retrieve the complete, permanent audit record for any completed action using the audit ID from webhook responses. Returns verified identity information, timestamps, action details, and any attached documents.
...
<ParamField path="auditID" type="string" required>
  ...
  **Where to get this:** Webhook responses from Sign API, Message API (with signature), and Identify API all include an `audit_id` field. Save this ID to query the full audit record later.
</ParamField>
