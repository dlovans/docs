...
UIP is **wallet-agnostic**. The user holds a credential issued by a government
authority; the wallet presents it over a standard protocol. Today that's the
**W3C Digital Credentials API** in the browser, carrying an **ISO/IEC 18013-5**
mobile-document (mdoc) — the same shape from both Apple and Google Wallet, so UIP
...
[Wallets & coverage](/core-concepts/wallets) for the current map and
[the roadmap](/roadmap) for what's coming.
