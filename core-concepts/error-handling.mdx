...
<Note>
**Rate Limits:** UIP enforces rate limits of 500/min per IP, 300/min per API key, and 100/min for authorize endpoints. See the [API Introduction](/api-reference/introduction) for details.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="request/invalid-path" icon="route">
    **HTTP 400** - The path parameter is invalid or missing
...
<AccordionGroup>
  <Accordion title="rate-limit/exceeded" icon="gauge-high">
    ...
    **Cause:**
    ...
    - Authorize endpoint: 100 requests per minute

    **Solution:**
    - Check `Retry-After` header for seconds until window resets
    - Check `X-RateLimit-Remaining` header to track remaining requests
...
<AccordionGroup>
  <Accordion title="auth/insufficient-scope" icon="shield-halved">
    **HTTP 403** - Insufficient scope for this operation

    **Cause:**
    - Delegation token does not have the required scope for this API call
    - The business granted fewer scopes than needed

    **Solution:**
    - Check the scopes granted to your delegation token
    - Request a new authorization with the needed scopes
    - Use a direct API key instead if you have one
  </Accordion>

  <Accordion title="auth/scope-not-allowed" icon="ban">
    **HTTP 400** - One or more requested scopes are not allowed

    **Cause:**
    - Invalid scope name in the authorize request
    - Requesting a scope that doesn't exist

    **Solution:**
    - Valid scopes: `identify:create`, `sign:create`, `messages:create`, `messages:read`, `audits:read`, `business:read`
