...
<Steps>
  ...
  <Step title="Poll for Completion">
    Poll `GET /v1/identify/:id` every 2-3 seconds until status is `completed`.
...
<Steps>
  ...
  <Step title="Initiate UIP Setup">
    ...
    Show QR code or "Open UIP" button depending on device type.
  </Step>
  ...
  <Step title="Link UIP to Account">
    ...
    - Extract `uip_id` from the completed response
...
<Steps>
  ...
  <Step title="Request Step-Up">
    Call the [Identify API](/api-reference/endpoint/identify) to request biometric verification. Show the QR code or deep link button depending on device.
  </Step>

  <Step title="Poll and Verify">
...
<Check>SaaS platforms serving multiple businesses</Check>
<Check>Payroll, HR, or compliance platforms needing signatures from employees</Check>
<Check>Marketplace platforms managing identity verification for sellers</Check>
<Check>White-label solutions where clients don't integrate UIP directly</Check>

### How It Works

<Steps>
  <Step title="Request Authorization">
    Call `POST /v1/authorize` with your platform's client credentials and the scopes you need (e.g., `identify:create`, `sign:create`).
  </Step>

  <Step title="Business Approves">
    Display the QR code to the business owner. They scan it, review the requested permissions, and approve with biometric verification.
  </Step>

  <Step title="Poll for Token">
    Poll `POST /v1/authorize/:id/status` with client credentials until `completed`. You receive a delegation token (`uip_at_` prefix) in the response.
  </Step>

  <Step title="Make API Calls on Behalf of Business">
    Use the delegation token as a Bearer token to make API calls. Billing goes to the business, not the platform.

    ```bash
    curl -X POST https://api.uip.digital/v1/identify \
      -H "Authorization: Bearer uip_at_abc123xyz789..." \
      -H "Content-Type: application/json" \
      -d '{"intent": "Verify employee", "requested_data": ["first_name", "last_name"]}'
    ```
  </Step>
</Steps>

<Info>
See the [Platform Delegation Guide](/core-concepts/platform-delegation) for full details on scopes, token management, and security best practices.
</Info>
...
| Multi-business platform | **Pattern 5: Delegation** | Single integration serves all customers |
...
<CardGroup cols={3}>
  ...
  <Card
    ...
    icon="users-gear"
    href="/core-concepts/platform-delegation"
  >
