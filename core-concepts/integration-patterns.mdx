...
<Steps>
  ...
  <Step title="Request UIP Verification">
    ...
    **Response:**
    - `session_id`, `qr_code` (raw SVG markup, not base64 PNG), `expires_at`
  </Step>
  ...
  <Step title="Display UIP Prompt">
    ...
    **Mobile Flow:**
    - Display "Open UIP App" button linking to `uip://identify?sid={session_id}&exp={unix_seconds}`
    - Show loading state: "Waiting for biometric verification..."

    **Desktop Flow:**
    - Inline the raw `qr_code` SVG markup directly into the page
    - Show instructions: "Scan with UIP app to complete login"
    - Add loading indicator
  </Step>

  <Step title="Poll for Completion">
    Poll `GET /v1/identify/:id` every 2-3 seconds until status is `approved`.

    **On Success:**
    - Extract `target_uip_id` and `audit_id` from the response
