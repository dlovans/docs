...
<Steps>
  <Step title="Navigate to console">
    Visit uip.digital and click **"Developer Access"**
  </Step>
...
- **Signup bonus:** $20 in credits on your first business — plenty to test and implement your integration
...
<AccordionGroup>
  ...
  <Accordion title="Security Planning" icon="shield-halved">
    ...
    - [ ] Webhook signature verification implemented (for message flows)
  ...
  <Accordion title="Testing Strategy" icon="flask">
    - [ ] Use $20 signup credit for initial testing
...
<Steps>
  ...
  <Step title="Display QR code or deep link">
    Desktop: inline the raw SVG `qr_code` markup. Mobile: show an "Open UIP" button linking to `uip://identify?sid={session_id}&exp={unix_seconds}`
  </Step>
  ...
  <Step title="Poll for result">
    Your backend polls `GET /v1/identify/:id` every 2-3 seconds until status is `approved`
  </Step>

  <Step title="Receive verified data">
    When approved, the poll response carries the signer's `signer_snapshot`, `target_uip_id`, and `audit_id`
  </Step>
...
<Steps>
  ...
  <Step title="Poll for result">
    Poll `GET /v1/sign/:id` until status is `signed`
  </Step>

  <Step title="Store proof">
    Save the `audit_id` and `target_uip_id` from the signed response for permanent audit trail access
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="How do I test my integration?" icon="vial">
    **Use your $20 signup credit and your own UIP identity for testing.** Create your UIP identity during onboarding, then use it to test the complete authentication and signature flows.
...
<CardGroup cols={2}>
  <Card title="Use Your Signup Credit" icon="gift">
    Use your $20 signup credit to test your integration against real UIP infrastructure
  </Card>
